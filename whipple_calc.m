...
% d_crit=whipple_crit(t_w,rho_b,rho_p,v_n,s,sig);
% [t_b,t_w]=whipple_sizing(rho_b,rho_p,v_n,s,sig,d);
...
[t_b,t_w]=swhipple_sizing(rho_w,rho_b,rho_p,v_n,s,sig,d_design,c_nk,theta)
...
vv=1:.1:15;
for i=1:length(vv)
    dd(i)=swhipple_crit(0.24,0.05,rho_w,rho_b,rho_p,vv(i),s,sig,theta,m_nk);
end
plot(vv,dd)
ylim([0,2])
%% Whippple functions
function d_crit=whipple_crit(t_w,rho_b,rho_p,v_n,s,sig)
if v_n>=3+7.7
...
function [t_b,t_w]=whipple_sizing(rho_b,rho_p,v_n,s,sig,d)
c_w=0.16;
...
function [t_b,t_w]=swhipple_sizing(rho_w,rho_b,rho_p,v_n,s,sig,d,theta,m_nk)
c_w=8.84;
c_o=0.38; c_b=0.15;
M_p=4/3.*(d/2).^3*pi*rho_p;
t_b=c_b.*d*rho_p/rho_b;
...
function d_crit=swhipple_crit(t_w,t_b,rho_w,rho_b,rho_p,v_n,s,sig,theta,m_nk)
